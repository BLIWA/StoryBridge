...
/**
 * The How We Work page's body — everything below the server-only
 * `setRequestLocale`/`getSiteImage` in
 * apps/website/src/app/[locale]/how-we-work/page.tsx, which now just awaits
 * the field photo and renders this.
 */
export function HowWeWorkBody({ fieldPhoto }: { fieldPhoto: SiteImage | null }) {
  const t = useTranslations("HowWeWork");
  const step = useTranslations("HowWeWork.steps");
  ...
  return (
    <>
      <PageHero
        ...
        variant="arcs"
        glyph="“"
        eyebrow={<Editable path="HowWeWork.eyebrow">{t("eyebrow")}</Editable>}
        title={<Editable path="HowWeWork.title">{t("title")}</Editable>}
        standfirstSerif
        standfirst={<Editable path="HowWeWork.standfirst" multiline>{t("standfirst")}</Editable>}
      />
        ...
        {PROCESS_STEPS.map((s, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              ...
              >
                <Editable path={`HowWeWork.steps.${s.id}.title`}>{step(`${s.id}.title`)}</Editable>
              </h2>
              <div style={{ fontSize: "16px", lineHeight: "1.75", color: "#3E4650" }}>
                <Editable path={`HowWeWork.steps.${s.id}.body`} multiline>{step(`${s.id}.body`)}</Editable>
              </div>
              {s.body2 && (
                <div style={{ fontSize: "16px", lineHeight: "1.75", color: "#3E4650" }}>
                  <Editable path={`HowWeWork.steps.${s.id}.body2`} multiline>{step(`${s.id}.body2`)}</Editable>
                </div>
              )}
            </div>
            ...
            {s.listCount ? (
              ...
              >
                {Array.from({ length: s.listCount }, (_, k) => (
                  <div key={k} style={{ fontSize: "15.5px", lineHeight: "1.7", color: "#3E4650" }}>
                    <Editable path={`HowWeWork.steps.${s.id}.list.${k}`} multiline>{step(`${s.id}.list.${k}`)}</Editable>
                  </div>
                ))}
                <div style={{ fontSize: "15.5px", lineHeight: "1.7", color: "#002D62", fontWeight: 500 }}>
                  <Editable path={`HowWeWork.steps.${s.id}.listLast`} multiline>{step(`${s.id}.listLast`)}</Editable>
                </div>
              </div>
            ) : s.photo ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                <EditableImage slotId="how-we-work.field-photo">
                  {fieldPhoto ? (
                    ...
                    <img
                      src={fieldPhoto.url}
                      alt={fieldPhoto.alt}
                      style={{ aspectRatio: "3/2", objectFit: "cover", borderRadius: "8px", border: "1px solid #D8D1C7", width: "100%", display: "block" }}
                    />
                  ) : (
                    <div
                      style={{
                ...
                >
                  <Editable path={`HowWeWork.steps.${s.id}.aside`} multiline>{step(`${s.id}.aside`)}</Editable>
                </div>
              </div>
            ) : s.languageCard ? (
              <div
              ...
              >
                ...
                >
                  <Editable path="HowWeWork.languageCardNote" multiline>{t("languageCardNote")}</Editable>
                </div>
              </div>
            ) : (
              <div style={asideQuote}>
                <Editable path={`HowWeWork.steps.${s.id}.aside`} multiline>{step(`${s.id}.aside`)}</Editable>
              </div>
            )}
          </div>
      ...
      <div style={{ background: "#001838", position: "relative", overflow: "hidden" }}>
        ...
        >
          ...
          >
            <Editable path="HowWeWork.closingEyebrow">{t("closingEyebrow")}</Editable>
          </div>
          <div
            style={{
              fontFamily: "'Source Serif 4',serif",
          ...
          >
            <Editable path="HowWeWork.tagline" multiline>{t("tagline")}</Editable>
          </div>
          <div
            style={{ fontFamily: "'Source Serif 4',serif", fontStyle: "italic", fontSize: "24px", color: "#B57D49" }}
          >
            <Editable path="HowWeWork.closingNote">{t("closingNote")}</Editable>
          </div>
          <Link
            href="/contact"
          ...
          >
            <Editable path="HowWeWork.closingCta">{t("closingCta")}</Editable>
          </Link>
        </div>
      </div>
    </>
