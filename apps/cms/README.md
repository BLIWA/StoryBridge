...
- Copy `.env.example` to `.env.local` and fill in the web app config from the Firebase console once the `sotrybridge` project's web app is registered — the sign-in screen renders without it, but auth calls will fail until it's set.
